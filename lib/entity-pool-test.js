...
test("setComponent with same component twice isn't in search twice", function(t) {
  ...
  var id = pool.create();
  pool.setComponent(id, "name", "jimmy");
  pool.setComponent(id, "name", "bobby");
...
test("onRemoveComponent with callback is called on removing component", function(t) {
  ...
  var id = pool.create();
  var info = pool.addComponent(id, "info");
  info.location = "Detroit";
  function callback(entity, component, value) {
    t.equal(entity, id);
    t.equal(component, "info");
    t.equal(value.location, "Detroit");
  }
  pool.onRemoveComponent("info", callback);

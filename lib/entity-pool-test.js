...
test("onChangeComponent with callback is called on modifying existing component", function(t) {
	t.plan(6);
	...
	function callback(entity, component, value) {
		t.equal(entity, id);
		t.equal(component, "name");
		t.equal(value, "salazar");
	}
	pool.onChangeComponent("name", callback);
	pool.onChangeComponent("name", callback);

	pool.set(id, "name", "salazar");
});

test("onChangeComponent with callback isn't called on adding new component", function(t) {
	t.plan(3);
...
test("onRemoveComponent with callback is called on removing component", function(t) {
	...
	var id = pool.create();
	pool.set(id, "name", "jimmy");
	function callback(entity, component) {
		t.equal(entity, id);
		t.equal(component, "name");
	}
